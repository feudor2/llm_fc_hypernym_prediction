...
class SynSet:
    def __init__(self, synset_id, synset_name, synset_type, words, synset_names_all=[], definition=None):
        ...
        self.definition = definition  # Definition from XML, None if empty
        self.rels = {}
...
class RuWordNet(WordNet):
    ...
    def _load_synsets(self, wordnet_root, black_list_synsets=None, black_list_senses=None):
        ...
        for synset_type, synset_path in synsets_paths.items():
            ...
            for synset in list(root):
                ...
                # Extract definition (None if empty)
                definition = synset.attrib.get('definition', '')
                if definition == '':
                    definition = None
                
                if black_list_synsets is not None and synset_id in black_list_synsets:
                    self.synsets[synset_id] = SynSet(synset_id, synset_name, synset_type, set(), definition=definition)
                    continue
                synset_words = set()
                ...
                self.senses.update(synset_words)
                self.synsets[synset_id] = SynSet(synset_id, synset_name, synset_type, synset_words, definition=definition)
                for sense in synset_words:
                    if sense not in self.sense2synid:
                        self.sense2synid[sense] = []
                    self.sense2synid[sense].append(synset_id)
    ...
    def get_hyponyms(self, synset_id=None, pos=None):
        """
        Get hyponyms for a given synset or all root synsets.
        
        This function is designed for LLM function calling to explore taxonomy structure.
        
        Args:
            synset_id (str, optional): The ID of the synset to get hyponyms for.
                                       If None, returns all synsets without hypernyms (root nodes).
            pos (str, optional): Part of speech filter. Can be 'N' (nouns), 'A' (adjectives),
                                 'V' (verbs). If None, returns all parts of speech.
        
        Returns:
            list[dict]: List of hyponyms, where each hyponym contains:
                - name (str): The name of the synset
                - words (list[str]): List of words in the synset (with underscores replaced by spaces)
                - id (str): The synset ID
                - hyponyms (list[str]): List of hyponym names for this synset
        
        Example:
            >>> # Get all root nodes
            >>> roots = wordnet.get_hyponyms(None)
            >>> # Get only noun root nodes
            >>> noun_roots = wordnet.get_hyponyms(None, pos='N')
            >>> # Get hyponyms of a specific synset
            >>> hyponyms = wordnet.get_hyponyms('some-synset-id')
            >>> # Get only noun hyponyms of a specific synset
            >>> noun_hyponyms = wordnet.get_hyponyms('some-synset-id', pos='N')
        """
        result = []
        
        if synset_id is None:
            # Return all synsets that have no hypernyms (root nodes)
            for sid, synset in self.synsets.items():
                # Filter by POS if specified
                if pos is not None and synset.synset_type != pos:
                    continue
                    
                if 'hypernym' not in synset.rels or len(synset.rels['hypernym']) == 0:
                    # Get hyponym names for this synset
                    hyponym_names = []
                    for hypo_id in synset.rels.get('hyponym', []):
                        if hypo_id in self.synsets:
                            hyponym_names.append(self.synsets[hypo_id].synset_name)
            ...
            # Return hyponyms of the specified synset
            if synset_id not in self.synsets:
                return []  # Synset not found
            
            synset = self.synsets[synset_id]
            hyponym_ids = synset.rels.get('hyponym', [])
            
            for hypo_id in hyponym_ids:
                if hypo_id in self.synsets:
                    hypo_synset = self.synsets[hypo_id]
